...
pub fn main() {
    ...
    let window = video_subsystem.window("Brian's Brain", SCREEN_WIDTH, SCREEN_HEIGHT)
        .position_centered()
        .build()
        .unwrap();
    ...
    let brain_board = BrainBoard::new_random(BOARD_SIZE, BOARD_SIZE, P_BRAIN).unwrap();
    let life_board = LifeBoard::new_random(BOARD_SIZE, BOARD_SIZE, P_LIFE).unwrap();
    {
    let mut renderer = AutomataRenderer::new(&sdl_context, &mut canvas, &texture_creator, brain_board, FRAMES_PER_SECOND, brain_color).unwrap();
    ...
    println!("That's {} frames per second.", 
        frames as f32 / (duration.as_secs() as f32 + duration.subsec_millis() as f32 / 1000.0));
    }
    {
    let mut renderer = AutomataRenderer::new(&sdl_context, &mut canvas, &texture_creator, life_board, FRAMES_PER_SECOND, life_color).unwrap();
    let (frames, duration) = renderer.run().unwrap();
    println!("Ran {} frames in {}.{} seconds", frames, duration.as_secs(), duration.subsec_millis());
    println!("That's {} frames per second.", 
        frames as f32 / (duration.as_secs() as f32 + duration.subsec_millis() as f32 / 1000.0));
